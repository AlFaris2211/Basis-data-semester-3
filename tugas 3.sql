...
SELECT City FROM Customers UNION SELECT City FROM Product ORDER BY City;
...
SELECT * FROM Product WHERE Price > SOME (SELECT Price FROM Products WHERE Price > 20);
...
SELECT A. CustomerName AS CustomerName1, B.CustomerName AS CustomerName2, A CityFROM Customers A, Customers B WHERE A.CustomerID <> B.CustomerID AND A.City = B.City ORDER BY A.City;
