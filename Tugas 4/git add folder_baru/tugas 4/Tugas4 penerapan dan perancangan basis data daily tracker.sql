...
-- VIEW untuk Menampilkan Aktivitas Harian Maya dan Anak-anak
CREATE VIEW view_aktivitas_harian AS
SELECT 
    a.activity_id,
    p.nama AS nama_pengguna,
    CASE 
        WHEN a.user_or_child = 'user' THEN p.nama
        ELSE anak.nama_anak
    END AS nama_aktivitas_pemilik,
    a.jenis_aktivitas,
    a.durasi,
    a.tanggal,
    a.user_or_child
FROM aktivitas a
JOIN pengguna p ON a.user_id = p.user_id
LEFT JOIN anak ON a.child_id = anak.child_id;


-- VIEW untuk Menampilkan Jadwal Anak dan Total Aktivitas per Anak:
CREATE VIEW view_total_aktivitas_anak AS
SELECT 
    anak.nama_anak,
    COUNT(a.activity_id) AS total_aktivitas,
    SUM(a.durasi) AS total_durasi
FROM aktivitas a
JOIN anak ON a.child_id = anak.child_id
WHERE a.user_or_child = 'child'
GROUP BY anak.nama_anak;


-- VIEW untuk Menampilkan Profil Pengguna dan Informasi Umum:
CREATE VIEW view_informasi_pengguna AS
SELECT 
    p.user_id,
    p.nama,
    p.email,
    ...
    p.hobby
FROM pengguna p;
select * from view_informasi_pengguna;
